...
function validateRow(row, index) {
  ...
  if (!normalizeNullable(row.full_name)) {
    throw new Error(`Row ${index + 2} is missing full_name.`);
  }
}
...
async function main() {
  ...
  const sourceLabel = getArgValue("--source-label") ?? "Manual player CSV import";
  const notes =
    getArgValue("--notes") ??
    "Local player CSV import. No scraping or live website ingestion.";
  ...
  const rawPlayers = rows.map((row) => ({
    ...
    full_name: normalizeNullable(row.full_name),
    birth_date: normalizeDate(row.birth_date),
    position_raw: normalizeNullable(row.position_raw),
    shoots_catches_raw: normalizeNullable(row.shoots_catches_raw),
    source_updated_at: normalizeDate(row.source_updated_at),
    payload: {
      import_type: "local_player_csv",
      imported_from: path.relative(workspaceRoot, resolvedFilePath),
      raw_row: row
    }
  }));

  const result = await prisma.rawPlayer.createMany({
    data: rawPlayers,
    skipDuplicates: true
  });
  ...
  await prisma.rawImportBatch.update({
    ...
    data: {
      ...
      notes: `${notes} Inserted ${result.count} new raw player rows from ${path.basename(
        resolvedFilePath
      )}.`
    }
  });

  console.log(`Imported ${result.count} raw player rows into batch ${batch.id}.`);
